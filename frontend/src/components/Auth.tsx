  ...
  return (
    <div className='h-screen flex justify-center flex-col text-center'>
        <div className='flex justify-center flex-col'>
            ...
            <div className='text-4xl font-bold mb-1'>{type === "signin" ? "Login" : "Create an account"}</div>
            <div className='text-slate-500'>{type === "signin" ? "Don't have an account?" : "Already have an account?"} 
                <Link className= 'underline pl-2'to={(type === "signin") ? "/signup" : "/signin"}>
                    {(type === "signin") ? "Sign Up" : "Sign In"}
                </Link>
            </div>
            <div className='pl-40 pr-40 py-5 text-start'>
...
function LabelledInput({label, placeholder, onChange, type}: LabelledInputType) {
    return <div>
        ...
        <label className="text-sm text-start font-medium text-gray-900">{label}</label>
        <input onChange= {onChange} type={ type ||"text"} id="first_name" className="mb-3 bg-gray-50 border border-gray-300 text-gray-900 text-sm
